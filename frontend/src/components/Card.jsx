      ...
      <div className={`${item.type === 'YOUTUBE' && "h-72 py-3 flex flex-col  gap-2 p-2"} ${item.type === 'TEXT' && "w-full md:w-72 h-72 p-3 flex flex-col justify-between"} ${item.type === 'TWITTER' && "py-3 h-96 overflow-hidden max-h-[600px] flex flex-col gap-5 p-2"}  bg-gray-200 border-2 border-black shadow-md    rounded-md `}>
        ...
        <div className="flex justify-between  gap-2">
          ...
          <div className="flex gap-5">
              ...
              <button onClick={()=> setIsShare(!isShare)} className="cursor-pointer md:text-xl"><i className="fa-solid fa-share-from-square"></i></button>
              <button onClick={()=> handleDeleteTask(item._id)} className="cursor-pointer md:text-xl"><i className="fa-solid fa-trash"></i></button>
          </div>
          
        </div>
      </div>
      {isShare && <ShareCard handleClose={()=> setIsShare(!isShare)} value={item._id}/>}
    </>
  );
}
